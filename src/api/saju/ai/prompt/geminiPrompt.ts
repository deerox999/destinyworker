...
export const generateAnalysisPrompts = (params: PromptParams) => {
  ...
  const 출력형식지시 = 설정.출력형식(타겟년도, 이해도레벨);
  const 톤리마인더 = 어조옵션 === "유쾌한" ? "\n중요: 시작은 재치 있는 한 줄로, 각 섹션에 자연스러운 위트 1줄을 포함해주세요. (민감 주제는 완화)\n" : "";
...
작성 지침(중요):
- 각 질문은 평균 10~16문장 수준으로 충분히 풀어주세요. 핵심 주제(2~3개)는 16문장 이상도 허용합니다.
- 각 질문에서 아래 요소 중 '적절한 것'을 3가지 이상 선택해 구성하세요(항목 간 중복 없이):
  A) 근거(사주 데이터에서 2개 이상)
  B) 가상 시나리오(1~2개)
  C) 비유(1개, 과하지 않게)
...
- 마크다운 소제목/리스트/강조를 적절히 섞어 반복감을 줄이세요.`;
  ...
  userPrompt += `${톤리마인더}${특별추가질문(user, isDevelop)}`;
