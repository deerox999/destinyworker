...
`;
  if (해설유형 === "종합운세") {
    userPrompt += `
### **기본 분석 질문:**
* **사주 종합 브리핑:** 신강약/격국(특수격 포함)/조후/병약/사주의 균형을 위한 핵심 에너지(용신·희신)를 먼저 확정하고, 이 다섯 요소의 도출 근거를 3~5문장으로 요약 제시해주세요. 용신은 삶의 방향성을 제시하는 나침반과 같으며, 이를 어떻게 활용하는지는 본인의 선택과 노력에 달려 있음을 명확히 하세요.
...
* **전체적 특성 분석:** 오행 균형, 신강약, 격국을 판단하여 판단하게 된 근거를 해설해주세요.

* **조후 분석:** 내면의 균형과 관계의 온도를 분석해주세요. 개인이 내적으로 어떤 기질을 가지고 있는지, 그리고 타인과의 관계에서 어떤 태도를 보이는지 설명하고, 대운의 변화가 조후의 균형을 어떻게 바꾸는지에 따라 심리적 안정감이나 대인 관계의 변화가 어떻게 나타나는지 구체적으로 해설해주세요.
...
* **십성의 통근 및 허실 분석:** 천간에 뜬 십성이 지지에 통근했는지 여부를 파악하여, 그 십성이 나타내는 재능이나 성향이 얼마나 안정적이고 현실적인 힘을 가지는지 분석해주세요. 이는 단순한 길흉을 넘어, 개인이 어떤 분야에 노력을 집중해야 실질적인 성과를 낼 수 있는지에 대한 방향성을 제시하는 근거가 됩니다.

* **사주 특성 심층 분석:** 원국에 존재하는 주요 신살을 분석하고, 이 신살이 긍정적 혹은 부정적으로 발현될 수 있는 조건과 그에 따른 삶의 양상을 구체적으로 제시해주세요.

* **십이운성을 통한 인생 에너지 분석:** 일간을 기준으로 십이운성의 흐름을 분석하여, 각 시기별로 일간의 에너지가 어떻게 변화하는지 설명해주세요. 예를 들어 '건록(建祿)'이나 '제왕(帝旺)' 시기에는 주도적인 역할을 할 수 있는 반면, '쇠(衰)'나 '병(病)' 시기에는 재정비와 내실 다지기가 중요함을 강조해주세요.

* **궁성론 심화 분석:** 사주팔자의 각 위치(년/월/일/시)는 특정 사회적 관계(가족, 배우자, 자식 등)를 상징합니다. 각 궁에 어떤 십성이 놓여 있는지 분석하여, 개인의 삶에서 가족이나 특정 관계가 어떤 영향을 미치는지 구체적으로 설명해주세요.

* **운의 활용 전략:** 대운과 세운의 흐름을 읽고 삶에 적용하는 방법을 제시해주세요. 사주 분석을 통해 파악된 길한 운(용희신)이 들어올 때는 어떤 기회를 적극적으로 포착하고, 어떤 분야에 투자를 늘려야 하는지 구체적인 방향을 제시하고, 반대로 불리한 운(기신)이 들어올 때는 어떤 행동을 자제하고, 어떤 리스크를 관리해야 하는지 실질적인 조언을 제공해주세요.

* **변화와 갈등의 동적 분석:** 특정 대운이나 세운에서 들어오는 글자가 원국과 어떻게 상호작용하는지 분석해주세요. 예를 들어, 원국의 지지 충(沖)이 대운에서 해소되거나, 합(合)이 충(沖)으로 깨지는 상황 등을 설명하며, 그로 인한 인간관계, 건강, 재물 등의 변화를 구체적으로 예측해주세요.

* **대운별 상세 분석:** 제공된 모든 대운 구간(시작~종료)을 빠짐없이 해설하세요. 각 대운마다 천간/지지의 작용, 원국과의 상호작용, 시기적 특징, 핵심 조언을 최소 포함하세요. 용신/희신 관점에서 각 대운의 천간/지지가 원국에 미치는 영향을 상세히 설명해주세요.
...
* **종합 분석 및 전망:** 전 구간 분석 결과를 종합하여 인생의 주요 전환점과 기회가 찾아오는 시기를 명확한 근거와 함께 제시하고, 전체 대운 흐름을 통한 종합적 전망을 제공해주세요.

* **오해 방지 FAQ:** 사용자 질문과 위 해설을 바탕으로, 독자가 가질 수 있는 오해를 3~5개 선정하여 Q/A 형식으로 간단히 제시해주세요.
${highQuality ? `
추가 지시(중요):
