...
export class SajuAnalysisWorker implements DurableObject {
  ...
  private async handleSubmit(request: Request): Promise<Response> {
    try {
      ...
      const jobId = body.jobId || `job_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      const job: AnalysisJob = {
        ...
        pointsCost: this.getPointsCost(body.type || 'individual'),
  ...
  private async processJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      const result = await this.retryGeminiCall(ai, payload);
      ...
      const title = this.generateTitle(job);

      // DB에 저장
      const saveResult = await this.saveSajuAnalysis(job, text, title, analysisCompletedAt);
      ...
      job.result = {
        ...
        metadata: {
          ...
          response_type: this.getResponseType(job.type),
          ...(job.type === 'compatibility' && {
            person1_name: job.sajuData?.person1?.name,
            person2_name: job.sajuData?.person2?.name,
          }),
          ...(job.type === 'yearly_fortune' && {
            service_type: 'paid',
            fortune_type: job.fortuneType,
          }),
        },
        points: {
          deducted: job.pointsCost,
          remaining: null,
          message: null,
        },
      };
  ...
  private buildGeminiPayload(job: AnalysisJob): any {
    ...
    if (job.i18n && job.i18n !== "ko") {
      ...
      systemPrompt = job.systemPrompt || languagePrompts[job.i18n] || systemPrompt;
    }
    ...
    if (job.sajuData) {
      if (job.sajuData.person1 && job.sajuData.person2) {
        contents.push({
          ...
          parts: [
            {
              text: `궁합 분석용 사주 데이터:\n\n첫 번째 사람 (${job.sajuData.person1.name}):\n${JSON.stringify(job.sajuData.person1.sajuData, null, 2)}\n\n두 번째 사람 (${job.sajuData.person2.name}):\n${JSON.stringify(job.sajuData.person2.sajuData, null, 2)}`,
            },
          ],
        });
      } else {
  ...
  private async retryGeminiCall(ai: GoogleGenAI, payload: any, maxRetries = 3): Promise<any> {
  ...
  private generateTitle(job: AnalysisJob): string {
    ...
    if (job.sajuData) {
      if (job.sajuData.정보 && job.sajuData.정보.생년월일 && job.sajuData.정보.생년월일.이름) {
        title += ` ${job.sajuData.정보.생년월일.이름}님`;
      } else if (job.sajuData.person1 && job.sajuData.person1.정보 && job.sajuData.person1.정보.생년월일 && job.sajuData.person1.정보.생년월일.이름) {
        title += ` ${job.sajuData.person1.정보.생년월일.이름}님`;
        if (job.sajuData.person2 && job.sajuData.person2.정보 && job.sajuData.person2.정보.생년월일 && job.sajuData.person2.정보.생년월일.이름) {
  ...
  private async saveSajuAnalysis(
    ...
    try {
      ...
      if (job.sajuData) {
        ...
        } else if (job.sajuData.person1 && job.sajuData.person1.정보 && job.sajuData.person1.정보.생년월일) {
          birthData = {
